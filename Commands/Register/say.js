...
if(!["966452128442880039"].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.react(Config.carpi)
...
const taggess = message.guild.members.cache.filter(m => m.user.username.includes("Shin")).size
const taggesss = message.guild.members.cache.filter(m => m.user.username.includes("SHİN")).size
...
\`>\` Tagımızda toplam \`${tagges+etikettag+taggess+taggesss}\` kullanıcı bulunmaktadır!
