...
exports.run = async (client, message, member) => {
   ...
   let yetkicek = data.fetch(`puan.${message.author.id}.yetkicek`)
   let yetkilicek = data.fetch(`yetkili.${message.author.id}.yetkilicek`)

        message.channel.send(new MessageEmbed()
        .setDescription(`${message.author} ***Kullanıcısının Yetkili Bilgileri***
     
    ${Config.cyber} **Genel Bilgileri**
        • Toplam erkek kayıt sayısı: \`${esayi}\`
        • Toplam bayan kayıt sayısı: \`${ksayi}\`
        • Toplam çektiği yetkili sayısı: \`${yetkilicek}\`
